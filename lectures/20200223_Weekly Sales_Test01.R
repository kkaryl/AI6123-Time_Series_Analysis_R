...
# Objective : Try to analyze time series data on weekly sales of tech videos
# Created by: Karyl
# Created on: 23 Feb 2020

# xy <- read.table('data/weekly sales of tech video.txt', header = F, dec =".")
# 
# xy <- convertRowsToList(xy)
x <- c(45.9, 45.4, 42.8, 34.4, 31.9, 36.6, 39.2, 41.4, 40.3, 43.1, 43.2
...
mean(x)
ts.plot(x)
acf(x)  # Does not cut off
pacf(x) # Cut off after lag 7

ts.yw <- ar.yw(x, order.max = 5) # Suggest order 2
ts.yw 
summary(ts.yw)

ar2 = arima(x, order = c(2, 0, 0)) # Model using AR(2)
ar2
